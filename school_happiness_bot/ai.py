...
_CITY_PATTERN_IZ = re.compile(r"(?:я\s+)?и[з3]\s+(?:города\s+)?(.+)", re.IGNORECASE)
# Запасной вариант для "в/под Москве" — берём только одно слово-название, без падежа.
_CITY_PATTERN_LOCATIVE = re.compile(r"(?:^|\s)(?:в|во|под)\s+([А-ЯЁ][а-яё\-]+)")
...
def _fallback_extract_city(text: str) -> str:
    match = _CITY_PATTERN_IZ.search(text)
    if match:
        candidate = match.group(1).strip(" .!,:;-")
        if candidate:
            return candidate

    match = _CITY_PATTERN_LOCATIVE.search(text)
    if match:
        candidate = match.group(1).strip(" .!,:;-")
        if candidate:
            return candidate
...
async def extract_name(text: str) -> str:
    ...
    system_prompt = (
        "Пользователь Telegram-бота отвечает на вопрос «Как вас зовут?». "
        "Ответ может быть каким угодно: просто имя, фраза вроде «меня зовут "
        "Иван» (возможно, с опечатками), неформальная реплика с вводными "
        "словами и частицами («да я Ленка», «ну Саша», «короче, Вова») или "
        "шутливое рифмованное прозвище («Ленка-пенка», «Вовка-морковка»). "
        "Верни ТОЛЬКО настоящее личное имя (и фамилию — но лишь если это "
        "явно реальная фамилия, а не рифмованная шутливая добавка или "
        "бессмысленное слово). Не включай вводные слова, частицы, кавычки, "
        "знаки препинания и рифмованные добавки. Если явного имени в "
        "сообщении нет, верни исходный текст без изменений."
    )
...
async def extract_city(text: str) -> str:
    ...
    system_prompt = (
        ...
        "города?». Ответ может быть каким угодно: просто название города, "
        "фраза вроде «я из города Москвы» (возможно, с опечатками), город "
        "в любом другом падеже («живу в Москве», «где-то под Казанью») или "
        "неуверенная неформальная реплика («фиг знает, наверное, в Москве "
        "сейчас»). Верни ТОЛЬКО название города в именительном падеже "
        "(например «Москва», а не «Москве», «Москвы» или «в Москве»), без "
        "вводных слов, слов-паразитов, пояснений про неуверенность, кавычек "
        "и знаков препинания. Если явного названия города в сообщении нет, "
